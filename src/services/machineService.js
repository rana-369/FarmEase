...
export const getOwnerStats = async () => {
  try {
    const response = await api.get('/equipment/my-listings');
    const equipment = response.data || [];
    
    // Calculate stats from equipment list
    const totalMachines = equipment.length;
    const activeRentals = equipment.filter(e => e.isAvailable).length;
    const totalEarnings = equipment.reduce((sum, e) => sum + (e.totalEarnings || 0), 0);
    const pendingRequests = equipment.filter(e => !e.isAvailable).length;
